...
<DataGrid columns={columns} data={crewMembers} getRowKey={(row) => row.id} selectable />;
```
...
```tsx
<div className="frigate-monitor-span-2">{/* Content spanning 2 monitors */}</div>
```
...
function EngineeringConsole() {
  ...
  const handlePowerFailure = () => {
    alert.critical("POWER FAILURE", "Main power offline. Switching to auxiliary.");
  };
