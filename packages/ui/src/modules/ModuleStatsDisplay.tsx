...
export function ModuleStatsDisplay({
  ...
  const hasPowerStats = stats.max_energy !== undefined || stats.production !== undefined || stats.power_consumption !== undefined;
  const hasHeatStats = stats.heat_generation !== undefined || stats.cooling_capacity !== undefined;
  const hasPropulsionStats = stats.thrust !== undefined || stats.energy_consumption !== undefined || stats.max_speed !== undefined;
  const hasShieldStats = stats.max_shield_strength !== undefined || stats.recharge_rate !== undefined;
  const hasWeaponStats = stats.damage !== undefined || stats.range !== undefined || stats.fire_rate !== undefined;
  ...
  const knownKeys = new Set([
    'max_energy', 'production', 'power_consumption',
    'heat_generation', 'cooling_capacity',
    'thrust', 'energy_consumption', 'max_speed',
    'max_shield_strength', 'recharge_rate', 'recharge_delay', 'efficiency',
    'damage', 'range', 'fire_rate', 'projectile_speed',
    'mass', 'weight'
  ]);
  ...
  if (compact) {
    ...
    if (stats.max_energy) importantStats.push({ label: 'Energy', value: stats.max_energy, unit: 'MJ' });
    if (stats.production) importantStats.push({ label: 'Output', value: stats.production, unit: 'MW' });
    if (stats.thrust) importantStats.push({ label: 'Thrust', value: stats.thrust, unit: 'kN' });
    if (stats.max_shield_strength) importantStats.push({ label: 'Shields', value: stats.max_shield_strength, unit: 'pts' });
    ...
    return (
      ...
      >
        {importantStats.map((stat, idx) => (
          <span key={idx}>
            {stat.label}: <span style={{ color: 'var(--frigate-text-primary)', fontWeight: 600 }}>
    ...
    >
      ...
      {hasShieldStats && (
        <StatGroup title="Shield Systems">
          ...
          <StatItem label="Efficiency" value={stats.efficiency ? stats.efficiency * 100 : undefined} unit="%" />
        </StatGroup>
      )}
      ...
      {!hasPowerStats && !hasHeatStats && !hasPropulsionStats && !hasShieldStats && !hasWeaponStats && !hasPhysicalStats && otherStats.length === 0 && (
        <div
          style={{
            fontFamily: 'var(--frigate-font-mono)',
            fontSize: 'var(--frigate-font-small)',
