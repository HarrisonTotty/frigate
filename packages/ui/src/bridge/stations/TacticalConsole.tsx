...
export function TacticalConsole({
  ...
  const getContactTypeAbbr = (type: string): string => {
    switch (type) {
      case 'hostile': return 'HSTL';
      case 'friendly': return 'FRND';
      case 'neutral': return 'NEUT';
      case 'unknown': return 'UNKN';
      default: return type.substring(0, 4).toUpperCase();
    }
  };

  const getWeaponTypeAbbr = (type: string): string => {
    switch (type) {
      case 'energy': return 'ENRG';
      case 'kinetic': return 'KNTC';
      case 'missile': return 'MSSL';
      default: return type.substring(0, 4).toUpperCase();
    }
  };
  ...
  const toggleAutoFire = async (weaponId: string, enabled: boolean) => {
    try {
      const response = await fetch(`${apiBaseUrl}/v1/ships/${shipId}/weapons/${weaponId}/auto-fire`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--frigate-space-2)' }}>
              {contacts.map((contact) => (
                <div
                  ...
                  style={{
                    padding: '6px 8px',
                    backgroundColor: targetId === contact.id ? 'var(--frigate-surface-overlay)' : 'transparent',
                    border: '1px solid',
                    borderColor: targetId === contact.id ? 'var(--frigate-primary)' : 'var(--frigate-border-base)',
                    cursor: 'pointer',
                ...
                >
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '8px' }}>
                    ...
                    <Badge variant={getContactTypeColor(contact.type || 'unknown') as 'default' | 'success' | 'danger'}>
              ...
              return target ? (
                <div>
                  ...
                  <div style={{ 
                    ...
                    <div>
                      <span style={{ color: 'var(--frigate-text-tertiary)', fontSize: 'var(--frigate-font-tiny)', letterSpacing: '0.05em' }}>RNG: </span>
                      <span style={{ fontWeight: 700 }}>{Math.sqrt(target.x ** 2 + target.y ** 2).toFixed(1)}k</span>
                    </div>
                    <div>
                      <span style={{ color: 'var(--frigate-text-tertiary)', fontSize: 'var(--frigate-font-tiny)', letterSpacing: '0.05em' }}>TYPE: </span>
                      <Badge variant={getContactTypeColor(target.type || 'unknown') as 'default' | 'success' | 'danger'}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 'var(--frigate-space-3)' }}>
            {weapons.map((weapon) => (
              <div
                ...
                style={{
                  padding: '8px',
                  backgroundColor: selectedWeapon === weapon.id ? 'var(--frigate-surface-overlay)' : 'var(--frigate-surface-base)',
              ...
              >
                ...
                <div style={{ marginTop: 'var(--frigate-space-2)', display: 'flex', gap: 'var(--frigate-space-2)' }}>
                  <Button
                    variant="primary"
                    size="sm"
                    onClick={(e) => { e.stopPropagation(); fireWeapon(weapon.id); }}
                  ...
                  <Button
                    variant="secondary"
                    size="sm"
                    onClick={(e) => { e.stopPropagation(); toggleAutoFire(weapon.id, !weapon.auto_fire); }}
                    style={{ flex: 1 }}
                  >
