        ...
        <div className="max-w-md">
          ...
          {selectedPlayer && (
            <Panel title="Selection Event" variant="muted" className="mt-4">
              <pre className="text-xs">{JSON.stringify(selectedPlayer, null, 2)}</pre>
            </Panel>
          )}
        </div>
      );
    ...
    const Demo = () => {
      ...
      return (
        <div className="max-w-md">
          ...
          {selectedTeam && (
            <Panel title="Selection Event" variant="muted" className="mt-4">
              <pre className="text-xs">{JSON.stringify(selectedTeam, null, 2)}</pre>
            </Panel>
          )}
        </div>
      );
  ...
  render: () => {
    return (
      <div className="h-screen bg-background-900 p-4">
        <div className="mb-4">
          <h1 className="text-2xl font-bold text-primary-400 mb-2">HYPERION Mission Lobby</h1>
    ...
    return (
      <div className="h-screen bg-background-900 p-4">
        ...
        <div className="space-y-4">
          ...
          <LobbyView apiUrl={MOCK_API_URL} enableWebSocket={false} pollingInterval={10000} />
        </div>
      </div>
    );
  },
  ...
  render: () => {
    return (
      <div className="min-h-screen bg-background-900 p-2 sm:p-4 md:p-6">
        ...
        <LobbyView apiUrl={MOCK_API_URL} enableWebSocket={false} pollingInterval={5000} />
      </div>
    );
  },
  parameters: {
