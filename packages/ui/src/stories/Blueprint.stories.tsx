...
import { BlueprintWorkflow, BlueprintList, RoleAssignment, BlueprintReadiness } from "../lobby";
        ...
        <div className="max-w-md">
          ...
          {selectedBlueprint && (
            <Panel title="Selection Event" variant="muted" className="mt-4">
              <pre className="text-xs">{JSON.stringify(selectedBlueprint, null, 2)}</pre>
            </Panel>
          )}
        </div>
      );
    ...
    const Demo = () => {
      ...
      return (
        <div className="h-screen bg-background-900 p-4">
          <div className="mb-4">
            <h1 className="text-2xl font-bold text-primary-400 mb-2">Ship Blueprint Design</h1>
          ...
          <div className="mt-4 p-3 bg-background-800 rounded border border-primary-700">
            <div className="text-xs text-text-muted">
              ...
              <code className="px-1 py-0.5 bg-background-900 rounded">{MOCK_API_URL}</code>. The UI
              will show errors if the server is not available, but all interactions are functional.
            </div>
          </div>
        </div>
      );
      ...
      return (
        <div className="h-screen bg-background-900 p-4">
          <div className="mb-4">
            ...
            <p className="text-sm text-text-muted">Demonstration with simulated blueprint data</p>
          </div>
          ...
          <div className="space-y-4">
            <Panel title="Mock Data Info" variant="muted">
              <div className="text-sm space-y-2">
                ...
                <div>
                  <strong>Ship Classes:</strong> Battleship, Cruiser, Destroyer, Frigate, Corvette,
                  Scout, Carrier
                </div>
                <div>
                  <strong>Roles:</strong> Captain, Helm, Engineering, Communications, Science,
                  Weapons, Countermeasures
                </div>
              </div>
            </Panel>

            <BlueprintWorkflow apiUrl={MOCK_API_URL} currentPlayerId={currentPlayerId} />
          </div>
        </div>
      );
    };
