  ...
  const loadPlayers = async () => {
    ...
    try {
      ...
      if (error instanceof SyntaxError) {
        console.warn(
          "JSON parsing failed for /v1/players - server may not have implemented this endpoint yet"
        );
        setPlayers([]);
      } else {
    ...
    try {
      ...
      if (!contentType || !contentType.includes("application/json")) {
        alert.warning(
          ...
          "Player registration endpoint not yet implemented on server"
        );
        return;
      }
  ...
  if (loading && players.length === 0) {
    return (
      <Panel title="Player Selection" className={className}>
        <div className="text-center py-8 text-text-muted">Loading personnel roster...</div>
      </Panel>
    );
  }
      ...
      <Stack direction="column" gap={4}>
        ...
        {selectedPlayer && (
          <div className="p-3 bg-primary-900/20 border border-primary-600 rounded">
            ...
            <div className="text-lg font-bold text-primary-400">{selectedPlayer.name}</div>
            {selectedPlayer.team_id && (
              <div className="text-sm text-text-secondary mt-1">Team: {selectedPlayer.team_id}</div>
            )}
          </div>
        )}
            ...
            <div className="space-y-2 max-h-64 overflow-y-auto">
              {players.map((player) => (
                ...
                >
                  ...
                  {player.team_id && (
                    <div className="text-xs text-text-muted mt-1">Team: {player.team_id}</div>
                  )}
                </button>
              ))}
            </div>
        ...
        {/* Refresh button */}
        <Button onClick={loadPlayers} variant="ghost" size="sm" disabled={loading} fullWidth>
