...
describe("ModuleSlotBrowser", () => {
  ...
  describe("keyboard hints", () => {
    it("displays all keyboard navigation hints", () => {
      render(<ModuleSlotBrowser {...mockProps} />);
      const hints = screen.getByText(/SEARCH/);
      expect(hints.textContent).toContain("SEARCH");
      expect(hints.textContent).toContain("SORT");
      expect(hints.textContent).toContain("NAV");
      expect(hints.textContent).toContain("ADD");
    });
  });
  ...
  describe("footer component", () => {
    it("renders footer with muted text color", () => {
      render(<ModuleSlotBrowser {...mockProps} />);
      const footer = screen.getByText(/SEARCH/);
  ...
  describe("default values", () => {
    it("uses default maxBuildPoints if not provided", () => {
      render(<ModuleSlotBrowser apiUrl={mockProps.apiUrl} blueprintId={mockProps.blueprintId} />);
      expect(screen.getByText("BUILD POINTS: 0 / 100")).toBeDefined();
    });

    it("uses default installedModules if not provided", () => {
      const { container } = render(
        <ModuleSlotBrowser apiUrl={mockProps.apiUrl} blueprintId={mockProps.blueprintId} />
      );
