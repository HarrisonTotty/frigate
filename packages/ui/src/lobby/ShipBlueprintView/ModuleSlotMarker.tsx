  ...
  if (variant) {
    ...
    if (variant.additional_hp !== 0) {
      stats.push({
        label: "VARIANT HP",
        value:
          variant.additional_hp > 0
            ? `+${variant.additional_hp}`
            : variant.additional_hp.toString(),
      });
    }
  }
 ...
 */
function buildTooltipTags(slot: ModuleSlotMarkerProps["slot"], isEmpty: boolean): string[] {
  const tags: string[] = [];
...
export function ModuleSlotMarker({
  ...
  const tooltipDescription =
    variant?.description ?? variant?.desc ?? slot?.description ?? slot?.desc;
    ...
    >
      ...
      <div style={{ flex: 1, minWidth: 0, overflow: "hidden" }}>
        ...
        >
          {isEmpty
            ? "[SELECT MODULE]"
            : (variantName ?? (hasVariants ? "[SELECT MODULE]" : "[FIXED]"))}
        </div>
      </div>
