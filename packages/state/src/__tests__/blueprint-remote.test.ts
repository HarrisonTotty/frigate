...
describe('blueprint remote helpers', () => {
  ...
  it('addInstanceRemote - success replaces temp id with created instance', async () => {
    ...
    vi.stubGlobal('fetch', vi.fn(async () => ({ ok: true, json: async () => created })));

    const instance = await useFrigateStore.getState().addInstanceRemote('', blueprintId, 'slot-x', null);
    ...
    expect(instances.some((i: any) => typeof i.id === 'string' && i.id.startsWith('tmp-'))).toBe(false);
  });
  ...
  it('addInstanceRemote - network error rollbacks temp instance and throws', async () => {
    ...
    await expect(useFrigateStore.getState().addInstanceRemote('', blueprintId, 'slot-x', null)).rejects.toThrow();
  ...
  it('setInstanceVariantRemote - success returns true and leaves variant', async () => {
    ...
    const ok = await useFrigateStore.getState().setInstanceVariantRemote('', blueprintId, 'inst-3', 'v1');
    expect(ok).toBe(true);
  const inst = useFrigateStore.getState().uiBlueprints[blueprintId].instances.find((i: any) => i.id === 'inst-3');
  ...
  it('setInstanceVariantRemote - non-ok returns false but optimistic set remains', async () => {
    ...
    const ok = await useFrigateStore.getState().setInstanceVariantRemote('', blueprintId, 'inst-4', 'v2');
    expect(ok).toBe(false);
  const inst = useFrigateStore.getState().uiBlueprints[blueprintId].instances.find((i: any) => i.id === 'inst-4');
  expect(inst).toBeDefined();
