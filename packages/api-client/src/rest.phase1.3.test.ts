...
describe('BlueprintsResource (Phase 1.3)', () => {
  const resource = new BlueprintsResource(mockHttp as any);

  it('addModule calls correct endpoint and body', async () => {
    const req: AddModuleRequest = { module_slot_id: 'aux-support-system', variant_id: 'aux-support-system-mk1' };
    await resource.addModule('bp-123', req);
    expect(mockHttp.post).toHaveBeenCalledWith('/v1/blueprints/bp-123/modules', req, expect.any(Function));
  });
  ...
  it('updateModuleVariant calls correct endpoint and body', async () => {
    ...
    expect(mockHttp.patch).toHaveBeenCalledWith('/v1/blueprints/bp-123/modules/mod-456', req, expect.any(Function));
  });

  it('removeModule calls correct endpoint', async () => {
    await resource.removeModule('bp-123', 'mod-456');
    expect(mockHttp.delete).toHaveBeenCalledWith('/v1/blueprints/bp-123/modules/mod-456', expect.any(Function));
  });
});
